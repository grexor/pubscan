...
# release any lingering locks then reopen
conn_names = sqlite3.connect(DB_FILE_NAMES)
conn_names.execute("PRAGMA wal_checkpoint(TRUNCATE);")
conn_names.commit()
conn_names.close()
...
# drop and recreate only the names table, leave everything else alone
